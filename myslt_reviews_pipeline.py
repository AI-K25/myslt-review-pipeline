...
# -*- coding: utf-8 -*-
"""
myslt_reviews_pipeline.py

Automatically scrapes MYSLT app reviews (Google Play & App Store),
translates non-English reviews, classifies feature requests using
a zero-shot model, and saves results to CSV files.
"""
...
# ======================
def fetch_google_reviews(app_id='com.slt.selfcare', lang='en', country='lk'):
    ...
    if 'at' in df.columns:
        df.rename(columns={'at': 'date'}, inplace=True)
    df['source'] = 'Google Play'
    df = df[['userName', 'content', 'score', 'date', 'source']]
    df = df.dropna(subset=['content'])
    df.to_csv('myslt_reviews.csv', index=False)
    logging.info(f"Saved {len(df)} Google Play reviews to myslt_reviews.csv")
    return df

# ======================
# Scrape App Store reviews
# ======================
def fetch_appstore_reviews(app_id="1492064957", pages=2):
    ...
    df = pd.DataFrame(reviews)
    df = df[['userName', 'content', 'score', 'date', 'source']].dropna(subset=['content'])
    df.to_csv('myslt_appstore_reviews.csv', index=False)
    logging.info(f"Saved {len(df)} App Store reviews to myslt_appstore_reviews.csv")
    return df

# ======================
# Preprocess and translate reviews
# ======================
def preprocess_reviews():
    df_google = fetch_google_reviews()
    df_appstore = fetch_appstore_reviews()
    df = pd.concat([df_google, df_appstore], ignore_index=True)
    df['translated_content'] = df['content'].apply(translate_review)
    df['date'] = pd.to_datetime(df['date'], errors='coerce')
    df['year'] = df['date'].dt.year
...
def process_reviews(preprocessed_csv='myslt_preprocessed_reviews.csv'):
    df = pd.read_csv(preprocessed_csv)
    df = df.dropna(subset=['translated_content'])
    feature_reviews = []
    classified_results = []

    for _, row in df.iterrows():
        review_text = row['translated_content']
        if is_feature_request(review_text):
            feature_reviews.append({
                "original": row['content'],
            ...
            result = classify_feature_request(review_text)
            result.update({
                "original": row['content'],
                "translated": review_text,
                "score": row['score'],
